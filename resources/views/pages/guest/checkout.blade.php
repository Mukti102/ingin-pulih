                ...
                <div class="px-6 md:px-8 pb-8 md:pb-10">
                    <div class="space-y-6 md:space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                            <div
                                class="p-4 md:p-5 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-center">
                                <span
                                    class="text-[9px] md:text-[10px] uppercase tracking-widest font-bold text-slate-400 block mb-1">Psikolog</span>
                                <h3 class="font-bold text-slate-800 text-sm md:text-base leading-tight">
                                    {{ $booking->psycholog->name }}</h3>
                                <p class="text-[10px] md:text-xs text-brand font-bold mt-1 uppercase">
                            ...
                            <div
                                class="p-4 md:p-5 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-center">
                                <span
                                    class="text-[9px] md:text-[10px] uppercase tracking-widest font-bold text-slate-400 block mb-1">Waktu
                                    Sesi</span>
                        ...
                        <div class="space-y-3 px-1">
                            <div class="flex justify-between items-center text-sm md:text-base">
                                ...
                                <span class="text-slate-800 font-semibold text-right">Rp
                                    {{ number_format($booking->earning, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-xs md:text-sm">
                                <span class="text-slate-400">Biaya Layanan</span>
                                <span class="text-slate-800 font-medium text-right">Rp
                                    {{ number_format($booking->platform_fee, 0, ',', '.') }}</span>
                            </div>

                            <div
                                class="pt-4 mt-2 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
                                <div>
                                    <span
                                        class="block text-[10px] uppercase tracking-widest font-black text-slate-400">Total
                                        Tagihan</span>
                                    <span
                                        class="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter leading-none">
                                        Rp {{ number_format($booking->total_price, 0, ',', '.') }}
                                    </span>
                                </div>
                                <div class="w-full sm:w-auto text-left sm:text-right">
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold bg-cyan-50 text-cyan-600 border border-cyan-100 whitespace-nowrap capitalize">
                        ...
                        <div class="pt-2">
                            ...
                            <p
                                class="text-center text-[10px] text-slate-400 mt-4 flex items-center justify-center gap-1.5">
